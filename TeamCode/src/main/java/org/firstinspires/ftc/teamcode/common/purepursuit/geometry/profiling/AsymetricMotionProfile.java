...
public class AsymetricMotionProfile {
    ...
    // TODO put back to protected when done doing profile testing
    public void compute() {
    ...
    public MotionState calculate(double seconds) {
        ...
        if (seconds <= this.dt1) {
            ...
            acceleration = 0;
            velocity = 0;
            return new MotionState(finalPosition, velocity * Math.signum(distance), acceleration * Math.signum(distance));
        }

        return new MotionState(this.initialPosition + position * Math.signum(distance), velocity * Math.signum(distance), acceleration * Math.signum(distance));
    }
}
