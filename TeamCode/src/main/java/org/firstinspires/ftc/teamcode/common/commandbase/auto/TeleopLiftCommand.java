...
public class TeleopLiftCommand extends ParallelCommandGroup {
    public TeleopLiftCommand(Robot robot, int position, LiftSubsystem.STATE state) {
        super(
                ...
                new LiftPositionCommand(robot.lift, position, 6000, 2500, state.equals(LiftSubsystem.STATE.FAILED_EXTEND) ? 30 : 10, state.equals(LiftSubsystem.STATE.FAILED_EXTEND) ? 3000 : 2000, state)
        );
    }
}
